...
import {
  ...
  ProgressBar,
  Progress,
} from "./shipmentProgress.style"; // Import your styled components
...
const CheckoutStepper: React.FC<CheckoutStepperProps> = ({
  ...
  useEffect(() => {
    setMargins({
      ...
      marginRight:
        (stepRef.current[stepsConfig.length - 1]?.offsetWidth || 0) / 2,
    });
  }, [stepsConfig.length]);
      ...
      {stepsConfig.map((step, index) => (
        ...
        >
          <div className="step-number">
            {activeStep > index + 1 || activeStep == 4 ? (
              <TiTick size={24} />
            ) : (
              step.icon
            )}
...
const mapStateToStep = (
  state: string,
  isCancelled: boolean,
  transitEvents: { state: string }[],
): number => {
...
const ShipmentProgress: FC<{ data: ShipmentResponse }> = ({ data }) => {
  ...
  const activeStep = mapStateToStep(
    ...
    <Container>
      <TextContainer>
        <TextHolder isCancelled={isCancelled} isFinished={activeStep === 4}>
          ...
          <h2>{isCancelled ? t("CANCELLED") : t(data.CurrentStatus.state)}</h2>
        </TextHolder>
        <TextHolder>
          <p> {t("LAST_UPDATE")}</p>
