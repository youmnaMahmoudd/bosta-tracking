...
import { ShipmentAddress, ShipmentAddressHolder, ShipmentWrapper, StyledTable, TableHolder, Title } from "./shipmentDetails.style";
        ...
        <tbody>
          {data.TransitEvents.map((event, index) => (
            <tr key={index}>
              ...
              <td dir='ltr'>{new Date(event.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</td>
              <td>{t(event.state || '')}</td>
            </tr>
          ))}
        </tbody>
